...
Name: Muna Mohamed
CS230: Section 5
Data: fat_food_usa.csv
URL:https://cs-final-7ty6zrj5qgmjnh8ltbwcbf.streamlit.app/
"""
...
# Bar chart for the number of selected restaurants in each state
filtered_state_summary = (
    restaurant_locations.groupby("State")
    .size()
    .reset_index(name="Count")
)

# Bar Chart
if not filtered_state_summary.empty:
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(filtered_state_summary["State"], filtered_state_summary["Count"], color='skyblue')
    ax.set_title(f"Count of {restaurant_name} Restaurants by State", fontsize=16)
...
# Bar chart for the top 10 restaurants in the selected state
top_restaurants_summary = (
    state_data["Restaurant Name"].value_counts().head(10).reset_index()
)
top_restaurants_summary.columns = ["Restaurant Name", "Count"]

# Bar Chart
if not top_restaurants_summary.empty:
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(top_restaurants_summary["Restaurant Name"], top_restaurants_summary["Count"], color='skyblue')
    ax.set_title(f"Top 10 Restaurants in {selected_state}", fontsize=16)
    ax.set_xlabel("Restaurant Name", fontsize=14)
    ax.set_ylabel("Count in State", fontsize=14)
